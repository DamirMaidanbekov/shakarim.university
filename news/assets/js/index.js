...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Инициализируем навигацию для видео
    initVideoCarousel();
});
...
/**
 * Инициализация слайдера для блока видео
 */
function initVideoSlider() {
    console.log("Инициализация видео слайдера");
    ...
    console.log(`Найдено ${videoItems.length} видео элементов`);
    ...
    function updateVisibility() {
        console.log(`Обновление видимости, текущий индекс: ${currentIndex}`);
        ...
        for (let i = currentIndex; i < currentIndex + itemsPerPage && i < totalItems; i++) {
            ...
            console.log(`Показываем элемент ${i}`);
        }
        ...
        console.log(`Состояние кнопок: пред=${prevBtn.disabled}, след=${nextBtn.disabled}`);
    }
}

/**
 * Настройка навигации для видео
 */
function setupVideoNavigation() {
    console.log('Настройка навигации видео');
    ...
    if (!prevBtn || !nextBtn || videoItems.length === 0) {
        console.error('Не найдены элементы навигации видео');
        return;
    }
    
    console.log(`Найдено видео: ${videoItems.length}`);
    ...
    nextBtn.addEventListener('click', function(e) {
        e.preventDefault();
        console.log('Нажата следующая');
        if (currentIndex < videoItems.length - itemsPerView) {
            currentIndex++;
            updateVideoVisibility();
        }
    });
    ...
    function updateVideoVisibility() {
        ...
        // Показываем только текущие элементы
        for (let i = currentIndex; i < currentIndex + itemsPerView && i < videoItems.length; i++) {
            videoItems[i].style.display = 'block';
            console.log(`Показываем видео ${i}`);
        }
        
        // Обновляем состояние кнопок
        prevBtn.disabled = currentIndex === 0;
        nextBtn.disabled = currentIndex >= videoItems.length - itemsPerView;
...
// Функция для перемещения видео при клике на кнопки
function moveVideos(direction) {
    ...
    // Текущее смещение
    const currentOffset = track.style.transform 
        ? parseInt(track.style.transform.replace('translateX(', '').replace('px)', '')) 
        : 0;
    
    // Максимальное смещение влево (в отрицательных числах)
    const maxOffset = -(items.length - 3) * itemWidth;
    ...
    if (direction === 'prev') {
        ...
        newOffset = Math.min(0, currentOffset + itemWidth);
    } else {
        // Движение вперед (влево)
        newOffset = Math.max(maxOffset, currentOffset - itemWidth);
    }
    ...
    // Обновляем состояние кнопок
    updateVideoNavButtons(newOffset, maxOffset);
}

// Инициализация карусели видео
function initVideoCarousel() {
    const prevBtn = document.querySelector('.video-section .prev-btn');
    const nextBtn = document.querySelector('.video-section .next-btn');
    const track = document.getElementById('videoTrack');
    
    if (!prevBtn || !nextBtn || !track) return;
    
    // Изначально кнопка "назад" неактивна
    prevBtn.classList.add('disabled');
    prevBtn.disabled = true;
    
    // Проверяем, нужно ли активировать кнопку "вперед"
    const items = track.querySelectorAll('.video-item');
    if (items.length <= 3) {
        nextBtn.classList.add('disabled');
...
function updateVideoNavButtons(currentOffset, maxOffset) {
    const prevBtn = document.querySelector('.video-section .prev-btn');
    const nextBtn = document.querySelector('.video-section .next-btn');
    
    if (!prevBtn || !nextBtn) return;
    
    // Кнопка "назад" активна, только если мы не в начале
    prevBtn.disabled = currentOffset >= 0;
    prevBtn.classList.toggle('disabled', currentOffset >= 0);
    
    // Кнопка "вперед" активна, только если мы не в конце
