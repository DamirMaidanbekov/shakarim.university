...
function initCalendar() {
    ...
    // После рендеринга пересчитываем высоту для заполнения контейнера
    setTimeout(() => {
        const announcementsGrid = document.querySelector('.announcements-grid');
        if (announcementsGrid) {
            const gridHeight = announcementsGrid.offsetHeight;
            calendar.setOption('height', gridHeight - 80); // Вычитаем отступы
            calendar.render();
    ...
    // При изменении размера окна пересчитываем размеры и обновляем календарь
    window.addEventListener('resize', function() {
        const announcementsGrid = document.querySelector('.announcements-grid');
        if (announcementsGrid) {
            const gridHeight = announcementsGrid.offsetHeight;
            calendar.setOption('height', gridHeight - 80);
            calendar.render();
            
            // Добавляем звездочки после ресайза
            setTimeout(() => {
